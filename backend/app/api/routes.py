...
from app.pipeline.pipeline_runner import pipeline_runner
...
def get_data_sources_status():
    """Returns provider status for SAR, AIS, and Environmental data feeds."""
    return provider_manager.get_data_sources_status()

@router.get("/scenes")
def list_sar_scenes():
    """Lists available SAR satellite acquisitions in ingestion queue."""
    return provider_manager.satellite_provider.list_available_scenes()

@router.post("/pipeline/run")
def run_automated_pipeline_endpoint(scene_id: str = Query("SD-SAR-001")):
    """Executes the complete automated oil-spill investigation pipeline."""
    try:
        return pipeline_runner.run_automated_pipeline(scene_id)
...
@router.get("/demo")
def get_demo_scenario(scene_id: str = Query("SD-SAR-001")):
    """Returns complete end-to-end Demo Incident scenario generated by pipeline_runner."""
    result = pipeline_runner.run_automated_pipeline(scene_id)
...
def download_pdf_report(scene_id: str = Query("SD-SAR-001"), incident_data: Optional[Dict[str, Any]] = None):
    """Generates and serves a downloadable PDF investigation report for a given scene or custom incident payload."""
    if incident_data:
        demo_incident = incident_data
    else:
        demo_incident = get_demo_scenario(scene_id)
...
def download_csv_report(scene_id: str = Query("SD-SAR-001"), incident_data: Optional[Dict[str, Any]] = None):
